...
def membership_price_adjustment(market, tier, minutes):
    ...
    if market=='ANON_MARKET1':
        ...
        if tier==1:
            adjusted_ride_price = ((1 - T1_MEMBERSHIP_PCT_DISCOUNT) * (ANON_MARKET1_PRICE_PER_MINUTE * minutes))
        if tier==2:
            adjusted_ride_price = ((1 - T2_MEMBERSHIP_PCT_DISCOUNT) * (ANON_MARKET1_PRICE_PER_MINUTE * minutes))
        if tier==3:
            adjusted_ride_price = ((1 - T3_MEMBERSHIP_PCT_DISCOUNT) * (ANON_MARKET1_PRICE_PER_MINUTE * minutes))
...
for i in tqdm(range(N_ITERATIONS)):
    ...
    results_df0['initial_ride_revenue'] = 0.0
    results_df0['initial_ride_revenue_array'] = 0
    results_df0['adjusted_revenue'] = 0.0
    results_df0['adjusted_revenue_array'] = 0
    results_df0['total_minutes'] = 0.0
    results_df0['ride_minutes_array'] = 0
    ...
    for idx, row in results_df0.iterrows():
        ...
        results_df0.at[idx, 'initial_ride_revenue'] = total_initial_ride_revenue
        results_df0.at[idx, 'initial_ride_revenue_array'] = str(initial_ride_revenue_array)
        
        results_df0.at[idx, 'total_minutes'] = total_minutes
        results_df0.at[idx, 'ride_minutes_array'] = str(ride_minutes_array.tolist())
...
iteration_results_summary_df.to_csv('./data/iteration_results_summary.csv', index=False)
